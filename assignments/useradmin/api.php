...
class MyAPI extends API
{
    ...
    protected function add_user()
    {
       ...
       if ($this->method == "POST") {
			...
			$results = $this->mh->insert([$this->request]);
    ...
    protected function update_user()
    {
		...
		if ($this->method == "POST") {
            ...
            $results = $this->mh->update(["_id" => $this->request["_id"]], $this->request);
    ...
    protected function find_user()
    {
        ...
        foreach($this->request as $key => $val){
            $newdata[$key] = $this->clean_entry($val);
        }
        return $this->mh->query($newdata);
