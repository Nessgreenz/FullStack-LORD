...
class MyAPI extends API
{
	...
	public function get_Image(){
		$wdata = [];
		$result = $this->mh->query($wdata);
		return $result;
		
	}
	
	public function waldo_Location(){
		$wdata = [];
		$i = 0;
		...
		if($this->method=="GET")
		{
			...
			$result = $this->mh->query(["game_id"=>$id]);
			foreach($result as $data=>$value)
			{
				foreach($value as $data=>$data_val){
					$wdata[$i] = $data_val;
					$i++;
				}
			}
			...
			$d = sqrt((pow($waldox-$x,2))+(pow($waldoy-$y,2)));
